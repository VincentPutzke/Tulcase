...
const DEFAULT_STATE_KEY = 'tulcase.pipe.tagFilter';
...
export class PipeTagFilter {
    ...
    /**
     * @param stateKey distinct per surface so Pipelines and Packages keep
     *   independent selections (each still workspace-local, never synced).
     */
    constructor(
        private readonly state: vscode.Memento,
        private readonly stateKey: string = DEFAULT_STATE_KEY,
    ) {}
    ...
    async setSelected(tags: string[]): Promise<void> {
        ...
        await this.state.update(this.stateKey, clean);
